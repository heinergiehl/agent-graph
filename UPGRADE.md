...
## 0.16.0 To 0.16.1: Idempotent Claim-Token Migration

Update general applications to `heiner/agent-graph:^0.16.1`. The Filament Agentic Chatbot release must instead pin exact `0.16.1` in both its package requirement and release contract.

The 0.16.0 migration could issue a duplicate `ADD COLUMN` when an earlier timestamp-renamed RC or published migration had already created `agent_graph_node_executions.claim_token`, but the canonical migration filename was not recorded. PostgreSQL reported this as SQLSTATE `42701` and stopped the deployment.

0.16.1 makes that pending migration idempotent. On the configured AgentGraph connection and table it now:

1. creates a missing nullable `varchar(26)` claim-token column and verifies the resulting schema;
2. accepts an existing compatible column without rewriting data or issuing DDL; and
3. fails closed before mutation when type, length, nullability, default, auto-increment, or generated behavior is incompatible.

SQLite does not retain declared `varchar` length in its schema metadata, so the migration validates its logical `varchar` type and nullability there. PostgreSQL, MySQL/MariaDB, and SQL Server retain and validate the exact length.

Keep workers stopped while updating the lock and running migrations. A successful prior 0.16.0 migration needs no schema change. If the migration is still pending, rerun the normal reviewed deployment command after installing 0.16.1; do not delete migration records, drop the existing column, overwrite published migrations with `--force`, or edit the database merely to bypass validation.

Rollback is also conservative: it validates the observed definition first, no-ops when the table or column is absent, and preserves the column when another recorded timestamp variant of the same published migration owns it. A fresh standalone 0.16.1 migration still removes the column it introduced.

## Historical: 0.16.0 RCs To 0.16.0 Stable
...
0.16.1 is the current stable pre-v1 patch on the 0.16 minor line. General root applications can select `^0.16.1`; align consuming-package constraints with their own release architecture. The Filament Agentic Chatbot's immutable productive dependency closure requires exact `0.16.1`. This upgrade changes persistence adapter contracts and rejects unsafe resume requests. Ordinary `TaskRunner::once()` and public graph/run/resume method signatures remain unchanged.
...
4. Install the reviewed stable `0.16.1` build and adapted application in the isolated test or staging environment while execution remains paused. Verify the resolved source and the complete consuming application before production adoption.
