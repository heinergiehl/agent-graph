...
class InMemoryMemoryStore implements EnumerableMemoryStore
{
    ...
    public function listNamespace(array $scopes, string $namespace): array
    {
        ...
        foreach ($this->orderScopes($scopes) as $scope) {
            $scopeRecords = array_values(array_filter($this->memories, function (array $memory) use ($scope, $namespace): bool {
            ...
            usort($scopeRecords, fn (array $left, array $right): int => [$left['key'], $left['id']] <=> [$right['key'], $right['id']]);

            array_push($records, ...$scopeRecords);
        }
