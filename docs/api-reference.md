...
- `recover(string $runId, ?callable $onEvent = null, bool $collectEvents = false): RunResult` continues a `running` run from an accepted pending-resume marker or its latest durable checkpoint under the run lock. It can also redeliver a valid pending delay through the bound scheduler while leaving its stored state unchanged. Ordinary input waits and terminal runs are unchanged. Delay redelivery is unreleased after 0.16.0-rc.1; see [delay recovery](guides/delay-recovery.md).
