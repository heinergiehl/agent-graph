...
This guide describes the upgrade to stable AgentGraph `0.16.1`. General AgentGraph root applications may use `^0.16.1`, but the Filament Agentic Chatbot release architecture freezes an exact productive dependency closure. Its package manifest and release contract must therefore pin exact `0.16.1`, and the host lock must resolve that exact stable version. Patch 0.16.1 also prevents a duplicate-column failure when the compatible claim-token column already exists from an earlier timestamp variant. The original RC1 compatibility patch and its early test results remain below as historical evidence.
